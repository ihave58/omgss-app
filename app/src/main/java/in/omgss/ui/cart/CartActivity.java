...
public class CartActivity extends BaseActivity implements PaymentResultWithDataListener {
    ...
    private void updateCartAmount() {
        ...
        for (Orderdetail items : cartList) {
            ...
            discountedTotal = discountedTotal + (items.getQuantity() * items.getSaleprice());
        }
        ...
        if (discountValue > 0) {
            ...
            discountedTotal = itemTotal - discountValue;

            tvLabelCouponDiscount.setText("Coupon Discount");
            ...
            tvLabelCouponDiscount.setText("Discount");
            tvCouponDiscount.setText(getString(R.string.currency));
            tvCouponDiscount.append(String.format("%.2f", (itemTotal - discountedTotal)));
        }

        tax = discountedTotal * (taxPercent / 100);
        cartTotal = discountedTotal + tax;

        tvItemTotal.setText(getString(R.string.currency));
        tvItemTotal.append(String.format("%.2f", itemTotal));

        tvDiscountedTotal.setText(getString(R.string.currency));
        tvDiscountedTotal.append(String.format("%.2f", discountedTotal));
...
//                options.put("amount", 100);
    ...
    @OnClick({R.id.iv_toolbar_left, R.id.atv_apply_coupon, R.id.atv_remove_coupon, R.id.tv_view_coupons, R.id.tv_change_address, R.id.tv_proceed_to_checkout})
